...
var menuItemsUrl ="../json/menu-cat_";
...
var switchMenuToActive = function () {
  ...
  if (classes.indexOf("active") === -1) {         // active not present
    classes += " active";
...
function buildAndShowHomeHTML (categories) {
  ...
  fetch(homeHtml)
    .then((result) => {
      ...
      chosenCategoryShortName = "'" + chosenCategoryShortName + "'";    // to string inside ''
...
dc.loadMenuCategories = function () {
  showLoading("#main-content");
  fetch(allCategories)     // using fetch api
    .then((result) => {
      return result.json();
    }).then(buildAndShowCategoriesHTML)
    .catch((err)=> {
      console.log(err);
    });
};
...
dc.loadMenuItems = function (categoryShort) {
  ...
  fetch(menuItemsUrl + categoryShort + ".json")
    .then((result) => {
      return result.json();
    }).then(buildAndShowMenuItemsHTML)    // callback fn.
    .catch((err)=> {
      console.log(err);
    });
};
